...
Np=ceil(Ts*f0); % Number of periods
% Ts=(Np/f0); % Signal time length
Ns=ceil(Ts*Fs); % Number of signal samples
...
figure(3)
f=(0:NFFT-1)*Fs/NFFT;
plot(f,abs(fft(s,NFFT)));
title('Signal spectrum')

figure(4)
plot(f,abs(fft(sdc,NFFT)),'b--');
hold on
plot(f,abs(fft(sdcfilt,NFFT)));
...
dfile=get_data_sonarfile([dpath,'test.txt'],1); % Downloaded data
dfile(isnan(dfile))=[]; iall=find(dfile); dfile=dfile(1:iall(end));
% Npoints=1000;
ADC=transpose(dfile(1:end));
...
filt_ADCc=rescale_minC(filt_ADCc,100); % Rescale filter so smallest number is 1
Nadc=length(ADC);
